...
##    (which will be used to save inverse matrix latter, (the cached_data));
## 2. Provides function get() to obtain "raw" matrix (where you  need to find 
##    its inverse);
...
## verse matrix has been found; if yes, returns the result and ends. If not, the 
## inverse of x is calculated, saved to cached, and returned.
## --------------------------------------------------------------------------------
cacheSolve <- function(x) {
  m <- x$getImatrix()
  if(!is.null(m)){
    message("Cached data found. Obtaining result....")
    return(m)
  }
  else {
